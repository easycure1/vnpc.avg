...
#' @keywords internal
gibbs_multivariate_nuisance <- function(mpg_avg,
                                        Nb,
                                        mcmc_params,
                                        corrected = F,
                                        prior_params,
  ...
  # d <- ncol(data)
  # n <- nrow(data)
  if (dim(mpg_avg)[3] %% 2 == 0) { # Not work for odd n
    n <- dim(mpg_avg)[3] * 2
  } else {
    n <- (dim(mpg_avg)[3] - 1) * 2
  }
  ...
  stopifnot(!is.null(prior_params$Sigma)); stopifnot(class(prior_params$Sigma)[1]=="matrix");
  ...
  # Corrected likelihood parameters
  if (corrected) {
    stopifnot(!is.null(prior_params$f_param_avg))
    f_param_avg <- prior_params$f_param_avg
    stopifnot(!is.null(prior_params$sqrt_d))
    sqrt_d <- prior_params$sqrt_d
    if (sqrt_d) {
      f_param_avg_half <- sqrt_cube(f_param_avg, excludeBoundary=F)
    } else {
      f_param_avg_half <- chol_cube(f_param_avg, excludeBoundary=F)
    }
  } else {
    f_param_avg_half <- array(dim = c(1,1,1)) # trivial
  }
  ...
  for (i in i_s) {
    ...
    # call C++
    mcmc_i <- gibbs_multivariate_nuisance_cpp(mpg_avg=mpg_avg,
  ...
  for (isample in 1:length(keep)) {
    ...
    if (corrected) {
      ...
      q_sample <- get_f_matrix(U__phi[,,isample], r[,isample], Z[,isample], k[isample], db.list)
