...
// Evaluate the VARMA(p, q) spectral density at some frequencies in [0, pi)
// CPP-internal only. FUnctionality equivalent to psd_varma(...)
// Note only the spectral density is returned
arma::cx_cube psd_varma_cpp(NumericVector freq,
                            arma::mat ar,
                            arma::mat ma,
                            arma::mat sigma) {
  const int d = sigma.n_rows;
  const int N = freq.length();
  // Should we add condition checks here as how psd_varma() does?
  arma::cx_cube transfer_ar = transfer_polynomial(freq, -ar);
  arma::cx_cube transfer_ma = transfer_polynomial(freq, ma);
  arma::cx_cube psd = varma_transfer2psd_cpp(transfer_ar, transfer_ma, sigma);
  return psd;
}
