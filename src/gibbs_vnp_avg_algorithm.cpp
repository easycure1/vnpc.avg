...
* based on average periodogram
* under Whittle's likelihood and a Bernstein-Hpd-Gamma prior.
 * The algorithm is elaborated in ...
 * 
 * The C++ core is called from the gibbs_vnp_avg.R file, see also
 * the documentation of the gibbs_vnp_avg method for further details.
 * 
 *  
 * 
 */
...
#include <math.h> // M_PI
#include "gibbs_vnp_avg_algorithm.h"
...
//' @keywords internal
// [[Rcpp::export]]
List gibbs_multivariate_nuisance_cpp(arma::cx_cube mpg_avg,  // average periodogram
                                     arma::cx_cube f_param_avg_half, // square root of the spectral density matrix of a prefitted VAR model
                                     ...
                                     List dbList,
                                     bool corrected) {   // precomputed Bernstein polynomials
  ...
  f_store = lpost_bernsteinGammaWhittle(mpg_avg,
                                        Nb,
                                        FZ, 
                                        psd_current, 
                                        a_prior, 
  ...
  for (int i=0; i<Ntotal-1; ++i) {
    ...
    double f_k_star;
    try {
      f_k_star = lpost_bernsteinGammaWhittle(mpg_avg,
                                             Nb,
                                             FZ, 
                                             psd_k_star, 
                                             a_prior, 
    ...
    for (int l=0; l<L; ++l) {
      ...
      double f_Z_star;
      try {
        f_Z_star = lpost_bernsteinGammaWhittle(mpg_avg,
                                               Nb,
                                               FZ, 
                                               psd_Z_star, 
                                               a_prior, 
    ...
    for (int l=0; l<L; ++l) {
      ...
      if (r_star(l) < numerical_thresh) {
        ...
        try {
          f_r_star = lpost_bernsteinGammaWhittle(mpg_avg,
                                                 Nb,
                                                 FZ, 
                                                 psd_r_star, 
                                                 a_prior, 
    ...
    for (int l=0; l<L; ++l) {
      ...
      double f_U_star;
      try {
        f_U_star = lpost_bernsteinGammaWhittle(mpg_avg,
                                               Nb,
                                               FZ, 
                                               psd_U_star, 
                                               a_prior, 
    ...
    //     double f_NA_star;
    //     try {
    //       f_NA_star = lpost_bernsteinGammaWhittle(mpg_avg,
    //                                               Nb,
    //                                               FZ_star, 
    //                                               psd_current, 
    //                                               a_prior, 
